...
`codexctl` работает с тремя типами окружений:
...
Слоты (`slot`) — это числовые идентификаторы dev‑AI окружений, которыми управляет команда `manage-env`. Для каждого
...
- GitHub Actions‑workflow реагирует на этот лейбл, вызывает `codexctl manage-env ensure-slot/ensure-ready --env ai ...`
...
## 🛠️ 4. Рендер и применение манифестов
...
- загружает `services.yaml`;
- собирает контекст (`Env`, `Namespace`, `Project`, `Slot`, `EnvMap` и т.д.);
- рендерит все инфраструктурные и сервисные манифесты в один YAML;
- выводит результат в stdout или в файл (`--output`).

Полезно для отладки шаблонов без применения в кластер.
...
- рендерит стэк так же, как `render`;
...
- Типичный пример — см. раздел 4.2.

### 🧾 5.3. `render`

- Назначение: отрендерить манифесты без применения.
- Полезен для CI‑проверок и локальной отладки шаблонов.
...
- Назначение: удаление ресурсов стэка (инфраструктуры/сервисов) из кластера.
- Используется осторожно, обычно в сценариях очистки dev/dev‑AI окружений.

### 📊 5.6. `status`

- Назначение: показать сводку по состоянию стэка (окружения, слоты, registry).

### 🎛️ 5.7. `manage-env`

Группа команд для работы со слотами dev‑AI (`env=ai`):

- `manage-env ensure-slot` — находит или создаёт слот для issue/PR:

  ```bash
  codexctl manage-env ensure-slot --env ai --issue 123 --output json
  ```

- `manage-env ensure-ready` — готовит окружение слота:
  ...
  Выполняет зеркалирование, сборку образов, `apply` и дополнительные настройки (секреты, ingress).

- `manage-env cleanup` — удаляет окружение слота и записи состояния.
- `manage-env gc` — сборка мусора (старые слоты).
- `manage-env resolve` — найти слот/namespace по issue/PR.
...
- `prompt config` — рендерит конфиг Codex (из `codex.configTemplate` или `config_default.toml`).
- `prompt render` — рендер произвольного шаблона промпта (по пути или по kind/lang).
...
- `plan list-children` — вывести все дочерние Issue для планирующего Issue:
...
- Диагностика окружения (`kubectl`, доступ к registry, базовые проверки).
...
1. `manage-env ensure-slot --env ai --issue <N>` — выбрать/создать слот.
2. `manage-env ensure-ready --env ai --slot <SLOT> --prepare-images --apply` — развернуть инфраструктуру и сервисы.
...
## 🐳 8. Образ Codex‑агента (`examples/codex-agent/Dockerfile`)

В репозитории есть пример Dockerfile для образа агента:
...
- `kubectl`, `gh`, `jq`, `ripgrep` и другие утилиты;
